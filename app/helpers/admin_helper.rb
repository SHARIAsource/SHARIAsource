  ...
  def hierarchy_list(hash)
    hash.map do |item, children|
      ...
      actions = content_tag :div, class: 'small-4 columns actions' do
        concat link_to(t('edit'), url_for([:admin, item, action: :edit]), class:'tiny radius pill button')
        concat link_to(t('delete'), url_for([:admin, item]), class:'tiny radius alert pill button', method: :delete,
